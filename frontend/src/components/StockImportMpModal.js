...
const StockImportMpModal = ({ onClose, onImported }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await stockMpAPI.upload(formData);
      setResult(response.data);
      if (onImported) onImported(response.data);
    } catch (err) {
      setError(err?.response?.data?.error || "Erreur lors de l'importation MP.");
    } finally {
      setUploading(false);
    }
  };
        ...
        <div className="stock-import-modal__body">
          ...
          {result ? (
            <div className="stock-import-modal__success">
              <strong>✓ {result.imported} article(s) MP importé(s) avec succès.{result.skipped > 0 ? ` • ${result.skipped} ligne(s) ignorée(s).` : ''}{result.promotedTasks > 0 ? ` • ${result.promotedTasks} tâche(s) débloquée(s).` : ''}</strong>
            ...
            <div className="stock-import-modal__upload">
              <input
                ...
                id="stock-mp-import-file"
              />
              <label htmlFor="stock-mp-import-file" className="stock-import-modal__file-label">
...
export default StockImportMpModal;
