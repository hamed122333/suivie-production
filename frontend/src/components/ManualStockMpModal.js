...
const ManualStockMpModal = ({ onClose, onAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValidArticleCode(article)) {
      setError(`Code article non valide.`);
      return;
    }
    ...
    try {
      const response = await stockMpAPI.createManual({
        article,
      ...
      setSuccess(
        response?.data?.mode === 'new_product'
          ? 'Nouvelle matière première créée avec succès !'
          : 'Stock MP ajouté avec succès !'
      );
      if (onAdded) onAdded(response.data);
      setFormData({ article: '', quantity: '', designation: '', clientCode: '', clientName: '' });
    } catch (err) {
      setError(err?.response?.data?.error || "Erreur lors de l'ajout manuel MP.");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="modal-content stock-import-modal" onClick={e => e.stopPropagation()} role="dialog">
        ...
        <div className="stock-import-modal__body">
          <p className="stock-import-modal__hint" style={{marginBottom: "1.5rem"}}>
            Saisissez le code article. Le système détecte automatiquement si c'est une matière première existante (ajout stock) ou une nouvelle MP.
          </p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label htmlFor="mp-article" style={{fontWeight: '600', color: '#2d3748', fontSize: '0.9rem'}}>Article / Référence MP *</label>
              <input
                id="mp-article"
                name="article"
                type="text"
                list="existing-mp-articles"
                value={formData.article}
                onChange={handleChange}
                placeholder="Tapez le code article (ex: MP12345)"
                style={{ textTransform: 'uppercase', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e0' }}
                required
              />
              <datalist id="existing-mp-articles">
              ...
              <small style={{color: '#718096', fontSize: '0.8rem'}}>
                Référence libre (ex: MP12345, ACIER-01, etc.).
              </small>
              {matchedArticle && (
                <div style={{ fontSize: '0.82rem', color: '#4a5568', marginTop: '0.35rem' }}>
                  Article existant détecté: {matchedArticle.article} | Stock actuel: {Number(matchedArticle.quantity || 0)} pcs
                </div>
              )}
              {!matchedArticle && formData.article.trim() && (
                <div style={{ fontSize: '0.82rem', color: '#7c3aed', marginTop: '0.35rem' }}>
                  {waitingStockHints[`${formData.article || ''}`.trim().toUpperCase()]
                    ...
                    : 'Nouveau produit: il sera créé dans le stock MP.'}
                </div>
              )}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label htmlFor="mp-quantity" style={{fontWeight: '600', color: '#2d3748', fontSize: '0.9rem'}}>Quantité *</label>
              <input
                id="mp-quantity"
                name="quantity"
                type="number"
            ...
            {!matchedArticle && formData.article.trim() && (
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label htmlFor="mp-designation" style={{fontWeight: '600', color: '#2d3748', fontSize: '0.9rem'}}>Désignation</label>
                  <input
                    id="mp-designation"
                    name="designation"
                    type="text"
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label htmlFor="mp-clientCode" style={{fontWeight: '600', color: '#2d3748', fontSize: '0.9rem'}}>Code client</label>
                  <input
                    id="mp-clientCode"
                    name="clientCode"
                    type="text"
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label htmlFor="mp-clientName" style={{fontWeight: '600', color: '#2d3748', fontSize: '0.9rem'}}>Nom client</label>
                  <input
                    id="mp-clientName"
                    name="clientName"
                    type="text"
            ...
            <div className="stock-import-modal__actions" style={{ marginTop: '1rem' }}>
              ...
              <button type="submit" className="btn btn-primary" disabled={submitting || !formData.article || !formData.quantity}>
                {submitting ? 'Ajout...' : matchedArticle ? 'Ajouter au stock MP' : 'Créer matière première'}
              </button>
            </div>
          </form>
        </div>
...
export default ManualStockMpModal;
